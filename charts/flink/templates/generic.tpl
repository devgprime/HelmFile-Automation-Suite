...
- name: {{ template "name.release" . }}
  ...
  {{- range $k, $v := .Values.config.private }}
    - name: {{ $k | upper }}
      valueFrom:
        secretKeyRef:
          name: {{ template "name.release" $ }}-secret-config
          key: {{ $k }}
  {{- end }}
{{ include "helper.resources" .Values.app | indent 2}}
...
spec:
  type: {{ template "helper.serviceType" . }}
  selector:
  ...
  ports:
    - name: {{ .Values.service.port.name }}
      ...
      {{- if and (eq (include "helper.serviceType" .) "NodePort") .Values.service.port.nodePort }}
...
kind: Service
metadata:
{{- include "meta.metadata" (dict "Root" . "Name" "debug") | indent 2 }}
spec:
  type: NodePort
  selector:
    app: {{ template "name.chart" . }}
  ...
  ports:
    ...
    - name: {{ template "name.release" $ }}-debug-{{ $port }}
      targetPort: {{ $port }}
      port: {{ $port }}
    {{- end }}
{{- end }}
